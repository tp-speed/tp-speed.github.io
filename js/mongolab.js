...
MongoLab.prototype.oneDocument = function(database, collection, id, callback){
    database = database || '';
    collection = collection || '';
    id = id || '';
...
MongoLab.prototype.update = function(database, collection, query, data, callback){
    callback = callback || function(){};
    database = database || '';
    collection = collection || '';
    ...
    this.put(endpoint, '&q='+JSON.stringify(query), data, callback);
};
...
MongoLab.prototype.put = function(endpoint, params, data, callback){

    callback = callback || function(){};
    params = params || '';
    data = typeof data === 'object' ? data : false;

    this.request(this.apiUrl+'/'+endpoint+'?apiKey='+this.apiKey+params, 'PUT', JSON.stringify(data), callback);
};
